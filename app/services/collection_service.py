...
class CollectionService:
    ...
    def create_collection(db: Session, collection_in: CollectionCreate):
        # Kiểm tra trùng tên
        existing = (
            db.query(Collection).filter(Collection.name == collection_in.name).first()
        )
        if existing:
    ...
    def get_list_collections(
        ...
        collections = (
            query.order_by(desc(Collection.created_at)).offset(skip).limit(limit).all()
        )
    ...
    def update_collection(
        db: Session, collection_id: UUID, collection_in: CollectionUpdate
    ):
        ...
        if collection_in.name is not None:
            ...
            if collection_in.name != collection.name:
                existing = (
                    db.query(Collection)
                    .filter(Collection.name == collection_in.name)
                    .first()
                )
                if existing:
                    raise HTTPException(
                        status_code=400, detail="Tên collection đã tồn tại!"
                    )
    ...
    def assign_documents(db: Session, collection_id: UUID, document_ids: List[UUID]):
        ...
        db.query(Document).filter(Document.id.in_(document_ids)).update(
            {Document.collection_id: collection_id}, synchronize_session=False
        )
    ...
    def sync_documents(db: Session, collection_id: UUID, document_ids: List[UUID]):
        ...
        db.query(Document).filter(Document.collection_id == collection_id).update(
            {Document.collection_id: None}, synchronize_session=False
        )
